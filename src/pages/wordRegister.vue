<template>
  <div class="loginContent">
    <t-space style="width: 350px" direction="vertical">
      ...
      <t-divider class="tip">注册</t-divider>
      <!--后期优化可以添加rule表单验证-->
      <t-form :data="formData" ref="form"  @submit="onSubmit" :colon="true" :labelWidth="0">
        ...
        <t-form-item>
          <t-button theme="primary" type="submit" block v-loading="loadingLogin" class="loginbutton">注册</t-button>
        </t-form-item>
      </t-form>
    </t-space>
  </div>
  ...
  methods: {
    onSubmit({ validateResult, firstError }) {
      ...
      if (validateResult === true) {
        register(this.formData.account,this.formData.password)
        .then(res=>{
          //通过res.data可以取用户信息
          console.log(res)
          //登录成功信息
